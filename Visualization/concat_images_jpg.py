...
def save_concat_image(destination_dir, image_name, image):
    ...
    cv2.imwrite('./'+destination_dir+image_name+'_concat.jpg', image)
...
image_name = 'fail_cases_material'
source_image_dir = './fail_cases_material/'
destination_dir = './material_fail_cases_concat/'
image_list_dir = source_image_dir

dim1 = 3
dim2 = 1
'''
concat_obj = concat(source_image_dir, destination_dir, image_list_dir, image_name, dim1, dim2)
concat_obj.concat_images(True)
'''
image_list = make_image_list(source_image_dir)
directory_list = ['./bridges_images/', './predicted_masks/', './combined_overlays/']
concat_lists(image_list, directory_list, './multi_concat/', pad=True, orientation=2)
